...
static int bt_init_power(void)
{
	...
	if (gpio_request(BTEN_GPIO, "BT enable") < 0) {
		printk(KERN_ERR "Failed to request GPIO%d for wl1271 bt enable\n",
			BTEN_GPIO);
		return -1;
	}

	addr = (u32 *) ioremap(0x48002168, 4);
	if (!addr) {
		printk(KERN_ERR
		       "OMAP3530_pad_config: ioremap failed with addr 0x48002168\n"
			);
		return -1;
	}
