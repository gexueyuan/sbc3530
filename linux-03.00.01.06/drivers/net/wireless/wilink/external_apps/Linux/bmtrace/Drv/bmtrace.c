...
unsigned long bm_act_trace_in()
{
	...
	if (ts <= bm_control.prev_param) {
		/* work around to avoid previous out_ts <= ts */
		ts = bm_control.prev_param + 1;
	}
...
int print_out_buffer(char *buf)
{
	...
	for (i = 0; i < g_position; i++) {
		...
		if (bm_entry[i].event_id & BM_PARAM_ENTRY_BIT) {
			len +=
			    sprintf(buf + len,
				    "TI-SoT: CLT_REC: UINT64: %s: %lu: %lu\n",
				    bm_trace_lut.lut[event_id].event_name,
				    bm_entry[i].ts, bm_entry[i].param);
		} else {
			len +=
			    sprintf(buf + len,
				    "TI-SoT: CLT_REC: %s: <->: %lu: %lu\n",
				    bm_trace_lut.lut[event_id].event_name,
				    bm_entry[i].ts, bm_entry[i].param);
		}
	}
...
static int print_proc(char *page, char **start, off_t off, int count, int *eof,
		      void *data)
{
	static int len;
	int new_count;
...
static void bm_test_simple_loop(unsigned char with_printing, int loop_count,
				int nop)
{
	/* this will mask the optimizer to cut out the loop */
	unsigned long num = 0x5a5;
	int i;
	...
	*/
		...
		p_buffer[i] = 1;
	{
		...
		while (--counter) {
			num += p_buffer[line_counter];
			/* move to the next line */
			line_counter += (PAGE_SIZE / sizeof(int));
			/* make sure you stay in the memory chunk */
			line_counter = 
			line_counter % (TLB_FAIL_MEM_REQUIRED / sizeof(int));
		}
		CL_TRACE_END_L0();
	}
...
*/
	...
	if (with_printing != False) {
		...
		printk("         assume: d-cache size is %d"
			"and d-cache line is %d\n", CACHE_SIZE,
			CACHE_LINE_SIZE);
		printk("         number of reads %d\n", NUM_OF_LOOP_TEST);
	}
	...
	if (with_printing != False) {
		...
		printk
		    ("         assume: %d TLBs in the CPU. Page size is %ld\n",
...
static void bm_act_print_lut(void)
{
	...
	for (i = 0; i < bm_trace_lut.counter; ++bm_trace_lut.counter)
		printk("lut[%d].event_name=%s", i,
		       bm_trace_lut.lut[i].event_name);
}
...
static void bm_act_clear_lut(void)
{
	...
	for (i = 0; i < bm_trace_lut.counter; ++bm_trace_lut.counter)
		BM_FREE(0, bm_trace_lut.lut[i].event_name,
			sizeof(bm_event_record_in_lut_t.event_name));

	BM_FREE(0, bm_trace_lut.lut,
		sizeof(bm_event_record_in_lut_t) * bm_trace_lut.counter);
...
int bm_act_register_event(char *module, char *context, char *group,
			  unsigned char level, char *name, char *suffix,
			  int is_param)
{
	...
	tmp_lut =
	    BM_ALLOC_IN_ISR(0,
			    sizeof(bm_event_record_in_lut_t) *
			    (bm_trace_lut.counter + 1), 0);
	/*printk("++++++++++ register function: %p\n", tmp_lut);*/
	/* Know copy the pointers */
	memcpy(tmp_lut, bm_trace_lut.lut,
	       sizeof(bm_event_record_in_lut_t) * bm_trace_lut.counter);
	BM_FREE_IN_ISR(0, bm_trace_lut.lut,
		       sizeof(bm_event_record_in_lut_t) * bm_trace_lut.counter);
	bm_trace_lut.lut = tmp_lut;

	bm_trace_lut.lut[bm_trace_lut.counter].event_name =
	    BM_ALLOC_IN_ISR(0,
			    strlen(module) + 2 + strlen(context) + 2 +
			    strlen(group) + 2 + 3 + 2 + strlen(name) + 2 +
			    strlen(suffix) + 2, 0);
	sprintf(bm_trace_lut.lut[bm_trace_lut.counter].event_name,
		"%s: %s: %s: %d: %s%s", module, context, group, level, name,
		suffix);
...
static void trace_ctl_usage(void)
{
	...
	printk("Use \"echo p|P > trace_ctl\" to reset logs, perform platform" 
		"tests, alt until reset\n");
	printk("Use \"echo a|A > trace_ctl\" to run full measurement of"
		" memory access\n");
}

/*----------------------------------------------------------------------------*/

static int trace_ctl_read(char *page, char **start, off_t off, int count,
			  int *eof, void *data)
{
	int len = 0;
	...
	len +=
	    sprintf(page + len,
		    "Use \"echo 0 > trace_ctl\" to disable the trace\n");
	len +=
	    sprintf(page + len,
		    "Use \"echo 1 > trace_ctl\" to enable the trace\n");
	len +=
	    sprintf(page + len,
		    "Use \"echo l|L > trace_ctl\" to print Look Up Table\n");
	len +=
	    sprintf(page + len,
		    "Use \"echo r|R > trace_ctl\" to reset the trace log\n");
	len +=
	    sprintf(page + len, "Use \"echo p|P > trace_ctl\" to reset logs,"
			" perform platform tests, alt until reset\n");
	len +=
	    sprintf(page + len, "Use \"echo s|S > trace_ctl\" to get 1000 clock"
			" cycles in nSec\n");
	len +=
	    sprintf(page + len, "Use \"echo a|A > trace_ctl\" to run full"
			" measurement of memory access\n");
...
static void bm_test_all(void)
{
	...
	printk("\nTrace enable\n");
	bm_enable();
	/* make sure that functions in cache, no printing */
	bm_measure_all(0);
	printk("\nReseting logs\n");
	bm_entry_pointer = bm_entry;
...
#define DISPLAY_REGISTER_VALUE(x, y)  printk("%-20s (0x%04x) = 0x%08X\n",#y, \
	(u32)&((typeof(x))0)->y, __REG32((u32)x + (u32)&((typeof(x))0)->y))

/*----------------------------------------------------------------------------*/

typedef unsigned int UINT32, *PUINT32;

static int trace_ctl_write(struct file *fp, const char *buf,
