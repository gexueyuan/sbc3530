			     ...
			     struct v4l2_input *inp)
{
	...
	if (vdev->vdev_sensor_mode) {
		if ((inp->index >= 0) && (inp->index < 4)) {
			strlcpy(inp->name, "COMPOSITE", sizeof(inp->name));
			inp->type = V4L2_INPUT_TYPE_CAMERA;
		} else if ((inp->index > 3) && (inp->index < 8)) {
...
static int vidioc_s_input(struct file *file, void *fh, unsigned int i)
{
	...
	if (vdev->vdev_sensor_mode) {
		...
		case 3:
			...
			break;
		case 4:
			route.input = INPUT_SVIDEO_VI2A_VI1A;
			break;
		case 5:
			route.input = INPUT_SVIDEO_VI2A_VI3A;
			break;
		case 6:
			route.input = INPUT_SVIDEO_VI4A_VI1A;
			break;
		case 7:
