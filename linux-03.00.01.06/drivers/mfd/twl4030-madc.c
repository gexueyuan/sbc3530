					      ...
					      u8 status_reg)
{
	...
	do {
		...
		if (!(reg & TWL4030_MADC_BUSY)
		    && (reg & TWL4030_MADC_EOC_SW))
			return 0;
...
int twl4030_madc_conversion(struct twl4030_madc_request *req)
{
	...
	if ((req->type == TWL4030_MADC_IRQ_ONESHOT)
	    && (req->func_cb != NULL)) {
	...
	ret =
	    twl4030_madc_wait_conversion_ready(the_madc, 5, method->ctrl);
	if (ret) {
	...
	ret =
	    twl4030_madc_read_channels(the_madc, method->rbase,
...
static int __init twl4030_madc_probe(struct platform_device *pdev)
{
	...
	madc->imr =
	    (pdata->irq_line == 1) ? TWL4030_MADC_IMR1 : TWL4030_MADC_IMR2;
	madc->isr =
	    (pdata->irq_line == 1) ? TWL4030_MADC_ISR1 : TWL4030_MADC_ISR2;
	...
	ret =
	    request_irq(platform_get_irq(pdev, 0),
